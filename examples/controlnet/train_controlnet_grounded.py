...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--gligen_captions_column",
        type=str,
        default="gligen_captions",
        help="The column of the dataset containing a caption or a list of captions.",
    )
    parser.add_argument(
        "--gligen_bboxes_column",
        type=str,
        default="gligen_bboxes",
        help="The column of the dataset containing a caption or a list of captions.",
    )
    parser.add_argument(
        "--max_train_samples",
...
def make_train_dataset(args, tokenizer, accelerator):
    ...
    image_transforms = A.Compose(
        [
            A.Resize(args.resolution, args.resolution,),# interpolation=transforms.InterpolationMode.BILINEAR),
            A.CenterCrop(args.resolution, args.resolution),
            #transforms.ToTensor(),
            A.Normalize(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5]),
            ToTensorV2()
        ], bbox_params=A.BboxParams(format='albumentations', min_area=1024, min_visibility=0.1, label_fields=['class_labels'])
    )

    conditioning_image_transforms = A.Compose(
        [
            A.Resize(args.resolution, args.resolution), # interpolation=transforms.InterpolationMode.BILINEAR),
            A.CenterCrop(args.resolution, args.resolution),
            #transforms.ToTensor(),
